...
class WebRTCClient: NSObject, RTCPeerConnectionDelegate, RTCVideoViewDelegate, RTCDataChannelDelegate {
    ...
    func connect(onSuccess: @escaping (RTCSessionDescription) -> Void){
        ...
        self.peerConnection?.setBweMinBitrateBps(200, currentBitrateBps: 300, maxBitrateBps: 400)
        if self.channels.video {
    ...
    func disconnect(){
        if self.peerConnection != nil{
            ...
            print("!!! -- WebRTC Peer connection is closed")
        }
    }
    ...
    func receiveCandidate(candidate: RTCIceCandidate){
        self.peerConnection!.add(candidate, completionHandler:{ result in
...
extension WebRTCClient {
    ...
    func peerConnection(_ peerConnection: RTCPeerConnection, didChange stateChanged: RTCSignalingState) {
        print("didChange stateChanged RTCSignalingState - \(stateChanged)")
        print("signaling state changed: ", stateChanged.rawValue)
    }
    
    func peerConnection(_ peerConnection: RTCPeerConnection, didChange newState: RTCIceConnectionState) {
        print("didChange newState RTCIceConnectionState - \(newState)  >>> \(RTCIceConnectionState.connected) | \( RTCIceConnectionState.completed) | \( isConnected)")
        switch newState {
    ...
    func peerConnection(_ peerConnection: RTCPeerConnection, didRemove candidates: [RTCIceCandidate]) {
        candidates.forEach{ candidate in
            print("didRemove candidate - \(candidate)")
        }
    }
    
    func peerConnectionShouldNegotiate(_ peerConnection: RTCPeerConnection) {
        print("peerConnectionShouldNegotiate - \( String(describing: peerConnection) )")
    }
